...
import { User, Lock, Mail, Phone, FileUp, FileDown, ChevronDown, Sun, Moon } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/register", {
        ...
        body: JSON.stringify({ username, name, email, password, phoneNumber: `${countryCode}${phone}` }),
      });
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      <div className="container" style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem 0" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          ...
          <p style={{ color: "var(--text-muted)" }}>Sign up to start converting files</p>
        </div>
              ...
              <div 
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") {
                    ...
                    if (match) {
                      const el = document.getElementById(`country-${match.code}`);
                      if (el) {
